...
def generate_payslip(emp_data, output_dir="output/payslips"):
    ...
    if till_short > 0:
        add_breakdown_row("Till Short", None, None, -till_short)
    if bonus > 0:
        add_breakdown_row("Bonus", None, None, bonus)

    gross = normal_amt + tue_amt + sunday_amt + hol_amt + leave_amt + sick_amt + bonus - till_short
    ...
    tips = float(emp_data.get('tips', 0.0))
    ...
    add_deduction_row("Tips", tips)
    add_deduction_row("UIF", uif)
    add_deduction_row("Clothing", clothing)

    total_deductions = tips + uif + clothing
    ...
    raw_name = emp_data.get('name', 'Unknown')
    # Remove invalid Windows filename characters
    safe_name = re.sub(r'[<>:"/\\|?*]', '', raw_name).strip().replace(' ', '_')
    filename = os.path.join(output_dir, f"{date_str}_{safe_name}_Payslip.pdf")
    pdf.output(filename)
    return filename
